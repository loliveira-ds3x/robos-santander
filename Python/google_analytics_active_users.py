from Packages.Main import GoogleAPI_main

api_type = 'simple_report'
crawler_name = 'google_analytics_active_users'
VIEW_ID = '236533316'
info_level = 'basic'
period = 'day'

report_period = [{'startDate': '2017-01-01', 'endDate': '2018-01-01'}]
report_dimensions = [{'name': 'ga:date'}]

report_metrics = [{'expression': 'ga:1dayUsers'},
                  {'expression': 'ga:7dayUsers'},
                  {'expression': 'ga:14dayUsers'},
                  {'expression': 'ga:28dayUsers'},
                  {'expression': 'ga:30dayUsers'}]

selected_columns = ['date','accountId_account','name_account', 'webPropertyId_wb' ,'name_wb'         ,'level'            ,'websiteUrl_wb'          ,'currency'        ,'type'        ,'websiteUrl_profile' ,'profileId_profile','name'        ,'1dayUsers', '7dayUsers', '14dayUsers'   ,'28dayUsers'    ,'30dayUsers','info', 'period']
column_names =     ['tempo''account_id'       ,'account_name', 'webproperty_id'   ,'webproperty_name','webproperty_level','webproperty_website_url','profile_currency','profile_type','profile_website_url','profile_id'       ,'profile_name','day_user' , 'week_user', 'two_week_user','four_week_user','month_user','info', 'period']


GoogleAPI_main.main(crawler_name=crawler_name, VIEW_ID=VIEW_ID, info_level=info_level, period=period, selected_columns=selected_columns, column_names=column_names,report_period=report_period,report_dimensions=report_dimensions, report_metrics=report_metrics)